...
file = ROOT.TFile.Open("/nfs/dust/cms/user/beozek/uuh2-106X_v2/CMSSW_10_6_28/src/UHH2/ZprimeSemiLeptonic/output_gen/UL16preVFP/muon/workdir_Analysis_UL16preVFP_muon_gen/nominal/uhh2.AnalysisModuleRunner.Semileptonic1.root")
...
hist4 = ROOT.TH1F("hist4", "DeltaY_N_gen_jet_muon", 1, -2, 2)
...
for i, event in enumerate(tree):
    ...
    hist4.Fill(event.DeltaY_N_gen_jet_muon)

        
    if i % 10000 == 0 and i > 0: 
        print("Processing entry {}/{}".format(i, tree.GetEntries()))

print("UL16preVFP muon -> Number of events without cuts: {}".format(hist1.Integral()))
print("UL16preVFP muon -> Number of events with lepton_pt cut: {}".format(hist2.Integral()))
print("UL16preVFP muon -> Number of events with lepton_pt & eta cut : {}".format(hist3.Integral()))
print("UL16preVFP muon -> Number of events with lepton_pt & eta & jet pteta cut : {}".format(hist4.Integral()))

# Create a new histogram with 3 bins
summary_hist = ROOT.TH1F("summary_hist", "UL16preVFP muon DeltaY < 0", 4, 0, 4)
...
canvas = ROOT.TCanvas("canvas", "UL16preVFP muon DeltaY < 0", 800, 600)
summary_hist.Draw()

canvas.Draw()
canvas.SaveAs("DeltaY_N_gen_muon_new.png")
