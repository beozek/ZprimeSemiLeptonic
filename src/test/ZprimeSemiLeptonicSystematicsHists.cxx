...
ZprimeChi2Discriminator::ZprimeChi2Discriminator(uhh2::Context& ctx){
  ...
  mtoplep_       = stof(ctx.get("chi2_mtoplep",       "173.6"));
  sigmatoplep_   = stof(ctx.get("chi2_sigmatoplep",   "24.6"));
  mtophad_       = stof(ctx.get("chi2_mtophad",       "173.0"));
  sigmatophad_   = stof(ctx.get("chi2_sigmatophad",   "21.2"));

  mtoplep_ttag_     = stof(ctx.get("chi2_mtoplep_ttag",     "171.4"));
  sigmatoplep_ttag_ = stof(ctx.get("chi2_sigmatoplep_ttag", "22.0"));
  mtophad_ttag_     = stof(ctx.get("chi2_mtophad_ttag",     "180.6"));
  sigmatophad_ttag_ = stof(ctx.get("chi2_sigmatophad_ttag", "15.6"));
...
TopPtReweighting::TopPtReweighting(uhh2::Context& ctx,
  ...
  TTbarEWKCorrection::TTbarEWKCorrection(uhh2::Context& ctx,
    ...
    // Evaluation and the POWHEG lookup filling both use signed cosTheta in [-1,1].
    // A Y axis starting at >= 0 means the table is binned in |cosTheta| or theta,
    // and both sides must be adapted before the results can be trusted.
    if(h_lo_qcd_->GetYaxis()->GetXmin() > -0.5){
      std::cerr << "TTbarEWKCorrection WARNING: EWno_0 Y axis is ["
                << h_lo_qcd_->GetYaxis()->GetXmin() << ", " << h_lo_qcd_->GetYaxis()->GetXmax()
                << "] but the lookup is evaluated with signed cosTheta in [-1,1]."
                << " Adapt the angle convention (|cosTheta| or theta) consistently." << std::endl;
    }
    // closes the file
    file_.Close();
  ...
  double TTbarEWKCorrection::evaluate_uncertainty(double mttbar, double costheta, double kappa) const {
    ...
    if(h_powheg_prediction_){
      ...
      // delta_QCD = 1 - sigma_LO/sigma_POWHEG. Both histograms must hold the same
      // quantity (same binning, same per-bin/density convention). Empty or invalid
      // POWHEG bins fall back to delta_QCD = 1, the full-correction envelope.
