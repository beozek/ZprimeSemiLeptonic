...
parser.add_argument('-outpath','--outpath', help = 'The name of the folder to be created in results', default = 'SH0016_Tunedto2000_Chip15', type = str)
parser.add_argument('-sensor','--sensor', help = 'The name of the sensor', default = 'SH0016_Tunedto2000_Chip15', type = str)
...
Sensor=args.sensor; thr_data_file='Run000011_SCurve.root'
# Path where the results will be stored
Path='results_xray/'
# Path to the NoiseScan root file (PixelAlive for us)
analyzed_data_file='Run000001_NoiseScan.root'; 
# Path to the .txt file that contains sensor information
analyzed_txt_file ='Run000011_ROC15.txt'
...
H_ID='0'; C_ID='15'; num_rows = 336; num_cols = 432; FIT=True
...
def XRayAnalysis(nTrg,nBX):
    ...
    ########## Set strange and problematic pixels as "good" (i.e., 3) as we don't want to see the problematics on the canvas-beren
    ...
    Missing_mat[Missing_mat != 1] = 3 # Keep only missing bumps (1); all others become good (3)
    ...
    #     print("({}, {})".format(Missing[1][i], Missing[0][i]))
    ...
    return Disabled[0].size, Data, Data_L, Missing_mat.T, Missing[0].size, ReadoutErrorsXRay, Perc_missing, ToTMapX
...
def Plots(ToTMap, NoiseMap, Noise_L, ThrMap, Thr_L, Data, Data_L, Missing_mat, Missing, Perc_missing, Disabled, ToTMapX, FitErrors):
    ...
    # Keep only the line for the threshold
    ...
    fig6.suptitle("Sensor " + Sensor + " -- Missing bumps (<" + str(Thr) + " hits): " + str(Missing) + " (" + str(Perc_missing) + "%)")
    ##beren
    ...
    # Missing Map: Show only missing bumps (highlight missing bumps)
    cmap = matplotlib.colors.ListedColormap(['red', 'white'])  # Show missing bumps in red
    bounds = [0, 1.9, 3.1]
    norm = matplotlib.colors.BoundaryNorm(bounds, cmap.N)
    
    
    imgplot2 = ax2.imshow(Missing_mat.T,cmap=cmap,norm=norm,origin='lower')
    ...
    plt.colorbar(imgplot2, ticks=bounds, orientation='horizontal', label='Missing (Red)      Good (White)', spacing='proportional', shrink=1)
    ...
    fig6.savefig(Path + Sensor + '/' + Voltage_1 + 'V_Missing_Bumps_Thr_' + str(Thr) + '.png', format='png', dpi=300)
...
def main():
    ...
    Disabled, Data, Data_L, Missing_mat, Missing, ReadoutErrorsXRay, Perc_missing, ToTMapX = XRayAnalysis(nTrg,nBX)
